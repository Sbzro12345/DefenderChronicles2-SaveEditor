#!/usr/bin/env python3
"""
Defender Chronicles 2 v1.2.1 save editor.

Finds com.gimka.defenderchronicles2.plist, decrypts the profile payload
...
SAVE_NAME = "com.gimka.defenderchronicles2.plist"

# Token Count sits 61 bytes before the General identifier:
TOKEN_BACK = 61
...
# not sit at the same place for all heroes
# New Structure: [16 bytes][GenSkill1-3][MelSkill1-3][LovSkill1-3][ElSkill1-3][ElSkill2][ElSkill3][48 bytes][Skill4][Skill5][Skill6]
...
HEROES = (
    ("General", b"General\x00", (
        ...
        ("Morale", 20),
        ("Cunning", 24),
        ("Infantry Commander", 112),
        ("Bowmen Commander", 116),
        ("Mage Commander", 120),
    )),
    ("Melwen", b"Melwen\x00", (
        ("Sorcery", 28),
...
# The General map array follows the closest [3C 72 65 73 65 74 3E 00 3C 72 65 73 65 74 3E 00] "reset" block above
# the Melwen identifier: ["reset" block][260 bytes][12 maps x 8 bytes]
# Melwen map array above Lovell, Lovell map array above Elwyn
# Elwyn above Beast King
ELWIN_ID = b"Elwyn\x00"
MAP_ANCHOR = b"\x01\x00\x00\x00\xff\xff\xff\xff"
MAP_ARRAY_AFTER = 304 #idk why this is offset by 44 bytes, perhaps AI didnt take into account difference of array size between DC1 and DC2?
MAP_ENTRY = 8
MAP_NAMES = (
    "Taric",
    "Tasaer",
...
TITLE = "Defender Chronicles 2 v1.2.1 save editor"
...
STARS_NOTE = """  
  ...
  their associated unlocks (e.g. Library Secrets' availability)
...
RANK_NOTE = """\
  ...
  For next difficulties to unlock, all must be at the L1 level.
  e.g. To unlock Immortal, all levels must minimum have 20 value.
  Otherwise, they will be capped at L!
  
  Mythic L1 = 0, Mythic L20 = 19
  Immortal L1 = 20, Immortal L20 = 39
  Demigod L1 = 40, Demigod L20 = 59
  Ultima L1 = 60, Ultima L20 = 79 ... and so on
  """
...
def build_maps(profile):
    """Locate the General map array from the anchor above the Melwen identifier."""
    melwen_id = b"Melwen\x00"
    melwen = profile.find(melwen_id)
    if melwen == -1:
        print("\nNo Melwen identifier in the profile payload, so the map array"
              " cannot be located. Nothing has been changed.")
        return None

    anchor = profile.rfind(MAP_ANCHOR, 0, melwen)
    if anchor == -1:
        print("\nNo map array anchor above the Melwen identifier."
    ...
    for i, name in enumerate(MAP_NAMES):
        ...
        maps.append({
            "name": name,
            "fields": [
                ("Number of Stars", base, (STARS_NOTE,)),
                ("Mythic+ L Number", base + 4, (RANK_NOTE,)),
            ],
        })
    return maps
...
def build_melwen_maps(profile):
    """Locate the Melwen map array from the anchor above the Lovell identifier."""
    lovell_id = b"Lovell\x00"
    lovell = profile.find(lovell_id)
    if lovell == -1:
        print("\nNo Lovell identifier in the profile payload, so the map array"
              " cannot be located. Nothing has been changed.")
        return None

    anchor = profile.rfind(MAP_ANCHOR, 0, lovell)
    if anchor == -1:
        print("\nNo map array anchor above the Lovell identifier."
              " Nothing has been changed.")
        return None
...
def build_lovell_maps(profile):
    """Locate the Lovell map array from the anchor above the Elwyn identifier."""
    elwin_id = ELWIN_ID # Uses the existing ELWIN_ID constant from the top of the file
    elwin = profile.find(elwin_id)
    if elwin == -1:
        print("\nNo Elwyn identifier in the profile payload, so the map array"
              " cannot be located. Nothing has been changed.")
        return None

    anchor = profile.rfind(MAP_ANCHOR, 0, elwin)
    if anchor == -1:
        print("\nNo map array anchor above the Elwyn identifier."
              " Nothing has been changed.")
        return None
...
def build_elwyn_maps(profile):
    """Locate the Elwyn map array from the anchor above the Beast King identifier."""
    beast_king_id = b"Beast King\x00"
    beast_king = profile.find(beast_king_id)
    if beast_king == -1:
        print("\nNo Beast King identifier in the profile payload, so the map array"
              " cannot be located. Nothing has been changed.")
        return None

    anchor = profile.rfind(MAP_ANCHOR, 0, beast_king)
    if anchor == -1:
        print("\nNo map array anchor above the Beast King identifier."
              " Nothing has been changed.")
        return None
...
def shift_bundle(bundle, pos, delta):
    ...
    for key in ("gmaps", "mmaps", "lmaps", "emaps"):
        if bundle.get(key):
...
def bulk_maps_page(profile, baseline, maps, title):
    ...
    while True:
        ...
        field_idx = choice - 1
        label = "Number of Stars" if choice == 1 else "Mythic+ L Number"
...
def heroes_page(profile, baseline, bundle):
    """Sub-menu for selecting which hero's stats to edit."""
    while True:
        header("HERO STATS")
        print(THIN)
        
        hero_names = ("General", "Melwen", "Lovell", "Elwyn")
        for n, name in enumerate(hero_names, 1):
            offsets = [off for _, off, _ in bundle["heroes"][name]["fields"]]
        ...
        name = hero_names[choice - 1]
        field_page(profile, baseline, "%s STATS \n" % name.upper(),
                   bundle["heroes"][name]["fields"], GUIDANCE)
                   
                   
def maps_editor_page(profile, baseline, bundle):
    """Sub-menu for selecting which hero's maps to edit."""
    while True:
        header("MAPS EDITOR")
        print(THIN)
        
        def get_map_offsets(maps):
            return [off for entry in maps for _, off, _ in entry["fields"]] if maps else []
            
        def p_row(n, label, maps_obj):
            if not maps_obj:
                print("  %d. %-24s (Not found)" % (n, label))
                return
            changed = count_pending(profile, baseline, get_map_offsets(maps_obj))
            note = ("(%d unsaved change%s)" % (changed, "" if changed == 1 else "s")) if changed else ""
            row = "  %d. %s" % (n, label)
            print(row.ljust(28) + note if note else row)
            
        p_row(1, "General Maps", bundle.get("gmaps"))
        p_row(2, "Melwen Maps", bundle.get("mmaps"))
        p_row(3, "Lovell Maps", bundle.get("lmaps"))
        p_row(4, "Elwyn Maps", bundle.get("emaps"))
        print("  5. Back")
        ...
        if choice == 1 and bundle.get("gmaps"):
            maps_page(profile, baseline, bundle["gmaps"], "GENERAL MAPS")
        elif choice == 2 and bundle.get("mmaps"):
            maps_page(profile, baseline, bundle["mmaps"], "MELWEN MAPS")
        elif choice == 3 and bundle.get("lmaps"):
...
def main_menu(profile, baseline, bundle, state, commit):
    ...
    while True:
        ...
        general_maps = bundle.get("gmaps")
        melwen_maps = bundle.get("mmaps")
        lovell_maps = bundle.get("lmaps")
        elwyn_maps = bundle.get("emaps")
        equipment = bundle.get("equip")
...
def main():
    ...
    lovell_maps = build_lovell_maps(profile)
    elwyn_maps = build_elwyn_maps(profile)
